...
- GitLab 브랜치, 커밋, 머지(MR)를 **Jira 이슈(Task, Story 등)에 자동 연결**하여 개발 이력을 명확하게 추적합니다.
- **커밋 메시지 또는 MR 제목에 이슈 키(S12P31B106-118)** 를 포함하면, 해당 이슈의 `Development` 패널에서 브랜치, 커밋, MR 기록이 자동 표시됩니다.
...
| 항목 | 내용 |
|------|------|
| 브랜치명 | `feature/S12P31B106-118-설명/front` 처럼 **Jira 이슈 키 포함 필수** |
| 커밋 메시지 | `[S12P31B106-118]feat: 기능 구현` 형식으로 작성 |
| MR 제목 | `[S12P31B106-118] 회원가입 기능 구현` 처럼 이슈 키 포함 |
